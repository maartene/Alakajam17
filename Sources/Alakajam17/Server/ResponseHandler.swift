...
final class ResponseHandler: ChannelInboundHandler {
    ...
    public func channelRead(context: ChannelHandlerContext, data: NIOAny) {
        ...
        responses.forEach { response in
            ...
            let ansiString = "\n" + AnsiFormatter.main.format(response.message) + "\n> "
            
            let sshAnsiString = ansiString.replacingOccurrences(of: "\n", with: "\r\n")
            
            var outBuff = context.channel.allocator.buffer(capacity: sshAnsiString.count)
            outBuff.writeString(sshAnsiString)
